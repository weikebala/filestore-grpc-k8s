...
import (
	...
	"filestore-grpc-k8s/service/dbproxy/orm"
	dbProto "filestore-grpc-k8s/service/dbproxy/proto"
)
...
var (
	dbCli dbProto.DBProxyServiceClient
)

func Init() {
	// 建立连接到gRPC服务
	conn, err := grpc.Dial(config.DbproxyServiceIngress, grpc.WithInsecure())
	if err != nil {
		log.Fatalf("did not connect: %v", err)
	}

	// 创建Waiter服务的客户端
	dbCli = dbProto.NewDBProxyServiceClient(conn)
}
